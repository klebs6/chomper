...
# rule postfix-expression-tail:sym<pp-mm> { [ <plus-plus> || <minus-minus> ] } #------------------------------------- #needs to stay like this for some reason.. #ie, cant be made proto without breaking some #parses, for example: # # uint8_t{format}
...
# rule new-expression { <doublecolon>? <new_> <new-placement>? [ || <new-type-id> || [ <.left-paren> <the-type-id> <.right-paren> ] ] <new-initializer>? }
...
# rule new-declarator { <pointer-operator>* <no-pointer-new-declarator>? } #applied a transfomation on this rule to #prevent infinite loops # #if we get any bugs downstream come back to #this
our class NewDeclarator { 
    #TODO
}

# rule no-pointer-new-declarator { <.left-bracket> <expression> <.right-bracket> <attribute-specifier-seq>? [ <.left-bracket> <constant-expression> <.right-bracket> <attribute-specifier-seq>? ]* } #------------------------
...
# rule pointer-member-expression { <cast-expression> [ <pointer-member-operator> <cast-expression> ]* } #-----------------
...
# rule multiplicative-expression { <pointer-member-expression> [ <multiplicative-operator> <pointer-member-expression> ]* } #-----------------
...
# rule additive-expression { <multiplicative-expression> [ <additive-operator> <multiplicative-expression> ]* }
our class AdditiveExpression { 
    #TODO
}

# rule shift-expression { <additive-expression> [ <shift-operator> <additive-expression> ]* } #-----------------------
...
# regex relational-expression { <shift-expression> [ <.ws> <relational-operator> <.ws> <shift-expression> ]* } #-----------------------
...
# rule equality-expression { <relational-expression> [ <equality-operator> <relational-expression> ]* }
...
# rule conditional-expression { <logical-or-expression> [ <question> <expression> <colon> <assignment-expression> ]? } #-----------------------
...
# rule pointer-declarator { [ <pointer-operator> <const>? ]* <no-pointer-declarator> } #------------------------------
...
# rule pure-specifier { <assign> <val=octal-literal> #|{if($val.text.compareTo("0")!=0) throw new InputMismatchException(this); } }
