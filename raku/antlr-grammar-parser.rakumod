...
#  Many language targets use {} as block
#  delimiters and so we must recursively match
#  {} delimited blocks to balance the
#  braces. Additionally, we must make some
#  assumptions about literal string
#  representation in the target language. We
#  assume
#  that they are delimited by ' or " and so
#  consume these in their own alts so as not
#  to inadvertantly match {}.
token ACTION
	{
	'{'	[	<COMMENT>
		|	<ACTION>
...
#  This is the list of all constructs that can be
#  declared before the set of rules that
#  compose the grammar, and is invoked 0..n
...
#  A list of options that affect analysis and/or
#  code generation
...
# Strings, actions and digit strings are all just
# scalar types, so rename the term, and take
...
#
# In the official ANTLR grammar, this is
# (probably) <ID> straight up. But, if we have
# an explicit <tokenName> rather than
# a generic <ID>, that can be given its own
# AST method and generate the simple block we
# want.
#
token tokenName { <ID> }
